#!/usr/bin/env python3
"""Build the ADK course textbook as a single-page HTML booklet from chapter markdown.

Adapted from barcik-training-publications/_sources/token-economics/tools/build_html.py.
Reads _sources/chapters/*.md (sorted), renders to textbook/index.html with sidebar nav."""
...
SRC_DIR = os.path.dirname(HERE)                        # textbook/_sources
CHAPTERS_DIR = os.path.join(SRC_DIR, "chapters")
TEXTBOOK_DIR = os.path.dirname(SRC_DIR)                # textbook/
OUTPUT_FILE = os.path.join(TEXTBOOK_DIR, "index.html")

BOOK_TITLE = "Google ADK — A Practical Course"
...
def md_to_html(text):
    return markdown.markdown(
        text,
        extensions=["tables", "fenced_code", "smarty", "attr_list"],
        output_format="html5",
    )
